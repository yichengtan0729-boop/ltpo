...
def _qwen_math_equal(label, answer):
    try:
        from .grader import math_equal_process
    except ImportError:
        return False
    return math_equal_process((label, answer))
...
def _extract_math_answer(text):
    try:
        ...
        return extract_MATH_solution(text)
...
def judge_answer(input, label, data_name="gsm8k", extract=True, prompt_idx=0):
    ...
    if "gsm8k" in data_name.lower() or "asdiv-aug" in data_name.lower():
        ...
        # huggingface math_verify, when installed
        ...
        # qwen2.5-math
        if _qwen_math_equal(label, input):
            return True
...
def extract_answer(text, data_name="gsm8k", prompt_idx=0, model_name="Qwen2.5-7B-Instruct"):
    ...
    if "gsm8k" in data_name.lower() or "asdiv-aug" in data_name.lower():
        ...
        if prompt_idx == 0 or prompt_idx == 2:
            # 0: boxed
            temp = _extract_math_answer(text)
            return temp
